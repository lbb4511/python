...
def set_proxy(keyList):
    if keyList.has_key('AutoConfigURL'):
        _winreg.DeleteValue(key, 'AutoConfigURL')
    if keyList['ProxyEnable'] != 1:
...
if __name__ == '__main__':
    ...
    keyList = get_process({})
    if not keyList.has_key(
            'AutoConfigURL'
    ) and keyList['ProxyEnable'] == 1 and keyList.has_key(
            'ProxyServer'
    ) and keyList[
            'ProxyServer'] == PROXY_PORT:  # determine whether proxy is well set
        # if it's well set, close the proxy and clear the proxy setting
        manage_shadowsocks(
            'create')  # shadowsocks will automatically change regs
