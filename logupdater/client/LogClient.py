...
class LogClient:
    ...
    def get_view_settings(self):
        r = self.s.get(self.baseUrl + '/worklog/worklogregister.aspx',
                       headers=self.headers)
        regex = {
        ...
        for key, value in regex.items():
            regex[key] = re.compile(value)
        self.viewSetting = {
            key: re.findall(value, r.text)
            for key, value in regex.items()
        }
    ...
    def upload_log(self, clientId, caseId, date, description, hours=0):
        try:
            payloads = {
                ...
                '__VIEWSTATEGENERATOR':
                self.viewSetting['__VIEWSTATEGENERATOR'],
            ...
            r = self.s.post(self.baseUrl + '/worklog/worklogregister.aspx',
                            data=payloads,
                            headers=self.headers)
            if DEBUG:
