...
class EvernoteController(object):
    ...
    def update_note(self, noteFullPath, content=None, fileDir=None):
        ...
        if note is None:
            return self.create_note(noteFullPath, content or '', fileDir)
        if '/' in noteFullPath:
            notebook = noteFullPath.split('/')[0]
            title = noteFullPath.split('/')[1]
        else:
    ...
    def get_attachment(self, noteFullPath):
        ...
        return {
            ...
            self.noteStore.getResourceData(resource.guid)
            for resource in (note.resources or {})
        }

    def move_note(self, noteFullPath, _to):
        if self.get(noteFullPath) is None: return False
        if type(self.get(noteFullPath)) != type(Types.Note()) or type(
                self.get(_to)) != type(Types.Notebook()):
            raise Exception('Type Error')
        self.noteStore.copyNote(self.token,
                                self.get(noteFullPath).guid,
                                self.get(_to).guid)
    ...
    def delete_note(self, noteFullPath):
        if self.get(noteFullPath) is None: return False
        if type(self.get(noteFullPath)) != type(Types.Note()):
