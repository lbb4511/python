...
def download_pic_from_url(url, picDir):
    ...
    try:
        ...
        postfix = 'jpg'
    with open((picDir + '.' + postfix).encode('cp936'), 'wb') as f:
        f.write(content)
...
def download(url, title, picUrl):
    for sk in (r'\/:*?"<>|'):
        title = title.replace(sk, '')
    title = title.replace('&nbsp;', ' ')
    if not os.path.exists(title.encode('cp936')):
        os.mkdir(title.encode('cp936'))
...
while 1:
    ...
    try:
        tkMessageBox.showinfo('Info',
                              u'读取成功，请打开"%s"文件夹查看' % TASK_STORAGE.pop())
    except:
        pass
