...
class MysqlClient:
    ...
    def restruct_table(self, tableName, orderBy, restructedTableName=None):
        ...
        orderByString = ', '.join(
            ['%s %s' % (key[0], key[1]) for key in orderBy])
        self.query(
            self.query('show create table %s' % tableName)[0][1].replace(
                tableName, restructedTableName, 1))
        sys.stdout.write('Restructuring the data storage...\r')
        totalNum = self.query('select count(*) from %s' % tableName)[0][0]
        s = self.data_source('select * from %s order by %s' %
        ...
        for data in s:
            insertSql = 'insert into %s values (%s)' % (
                restructedTableName, ', '.join(
                    ['%s' for i in range(len(data))]))
            ...
            if process < totalCount * 100 / totalNum:
                ...
                sys.stdout.flush()
                sys.stdout.write('Restructuring the data storage: %s%s\r' %
                                 (process, '%'))
            if count >= MAX_NUM:
                count = 0
                self._connection.commit()
    ...
    def simple_data_source(
            self, sql):  # return a function to provide one data at a time
    ...
    def parallel_get_source_of_data_source(self, sql, beginNumber=0):
        ...
        self._storedDataSource = self.simple_data_source(
            '%s limit %s, %s' % (sql, beginNumber, MAX_NUM))

        def get(i):
            self._storedDataSource = self.simple_data_source(
                '%s limit %s, %s' % (sql, i * MAX_NUM, MAX_NUM))

        if unitNumber == beginNumber / MAX_NUM: yield self._storedDataSource
        for i in range(beginNumber / MAX_NUM + 1, unitNumber + 1):
            while self._storedDataSource is None:
                print 'Thread sucks'
    ...
    def data_source(self, sql):  # limit is now useless here
        ...
        sourceOfDataSource = self.parallel_get_source_of_data_source(
            r[0],
