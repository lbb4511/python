...
class joystick(object):
    ...
    def __init__(self):
        for i in range(5):
            self.__statusDict['axis'][i] = 0
        for i in range(2):
            self.__statusDict['hat'][i] = 0
    ...
    def __switch_determine(self, part, number, status):
        if status != self.__statusDict[part][number]:
            if self.__statusDict[part][number] != 0:
                self.__functionDict[part].get(number, lambda x: None)(0)
            if status != 0:
                self.__functionDict[part].get(number, lambda x: None)(status)
